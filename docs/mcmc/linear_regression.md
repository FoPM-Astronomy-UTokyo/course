...
誤差は正規分布で近似できるという単純なモデルを採用します. まずは Y 軸 ($\log{M_B}$) の不定性だけを考えて尤度関数 (確率) を導出します.[^2][^3]
...
$\tau$ が負値を取らないように事前分布として Gamma 分布を仮定しました. 以下のサンプルでは無情報であることを意味するため $k$, $\theta$ にそれぞれ $10^{-3}$, $10^3$ を与えています.[^4]
